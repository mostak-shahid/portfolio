...
function site_identity_func( $atts = array(), $content = null ) {
	...
	$logo_option = $portfolio_options['logo-option'];
	$html = '';
	$atts = shortcode_atts( array(
		'class' => '',
		'container_class' => ''
...
function email_func( $atts = array(), $content = '' ) {	
	global $portfolio_options;
	$contact_email = $portfolio_options['contact-email'];
	$html = '';	
	$atts = shortcode_atts( array(
		'offset' => 0,
		'index' => 0,
...
function phone_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $portfolio_options['contact-phone'][$i]);
	    $html .= '" >';
	    $html .= $portfolio_options['contact-phone'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		foreach ($portfolio_options['contact-phone'] as $phone) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="phone-number">';
			    $html .= '<a class="phoneToShow" href="tel:';
...
function fax_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $portfolio_options['contact-fax'][$i]);
	    $html .= '" >';
	    $html .= $portfolio_options['contact-fax'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		foreach ($portfolio_options['contact-fax'] as $fax) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="fax-number">';
			    $html .= '<a class="faxToShow" href="tel:';
...
function address_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= '<span class="address-title">'.$portfolio_options['contact-address'][$i]['title'].'</span>';
		if ($portfolio_options['contact-address'][$i]['map_link']) :
			$html .= '<a class="address-details" href="'.$portfolio_options['contact-address'][$i]['map_link'].'" target="_blank">'.$portfolio_options['contact-address'][$i]['description'].'</a>';
		else :
			$html .= '<span  class="address-details">'.$portfolio_options['contact-address'][$i]['description'].'</span>';
		endif;
	    $html .= '</span>';
	else :
		foreach ($portfolio_options['contact-address'] as $address) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="address address-'.$n.'">';
...
function social_menu_fnc( $atts = array(), $content = '' ) {
	...
	global $portfolio_options;
	$html = '';
	$contact_social = $portfolio_options['contact-social'];
	$contact_address = $portfolio_options['contact-address'];
	$atts = shortcode_atts( array(
		'display' => 'inline',
		'title' => 0,
	), $atts, 'social-menu' );

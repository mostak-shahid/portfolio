    ...
    class ReduxFramework_mos_address {
        ...
        public function render() {
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                   ...
                   if ( empty ( $slide[ 'thumb' ] ) && !empty ( $slide[ 'attachment_id' ] ) ) {
                    ...
                    echo '<span class="button media_upload_button" id="add_' . esc_attr( $x ) . '">' . esc_html__( 'Upload Map Image', 'portfolio' ) . '</span>';
                    ...
                    echo '<li><input type="text" id="' . esc_attr( $this->field['id'] ) . '-title_' . esc_attr( $x ) . '" name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][title]" value="' . esc_attr($slide['title']) . '" placeholder="'.esc_html__('Map Title', 'portfolio').'" class="full-text slide-title" /></li>';
                    if ( $this->field[ 'show' ][ 'description' ] ) {
                        echo '<li><textarea name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][description]" id="' . esc_attr( $this->field['id'] ) . '-description_' . esc_attr( $x ) . '" placeholder="'.esc_attr__('Address', 'portfolio').'" class="large-text" rows="6">' . esc_attr($slide['description']) . '</textarea></li>';
                    }
                    ...
                    if ( $this->field[ 'show' ][ 'review_link_img' ] ) {
                        echo '<li><input type="text" id="' . esc_attr( $this->field['id'] ) . '-review_link_img_' . esc_attr( $x ) . '" name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][review_link_img]" value="' . esc_attr($slide['review_link_img']) . '" class="full-text" placeholder="'.esc_html__('Review Link Image', 'portfolio').'" /></li>';
                    }
            ...
            if ($x == 0) {
                ...
                echo '<ul id="' . esc_attr( $this->field['id'] ) . '-ul" class="redux-slides-list">';
                $placeholder = (isset($this->field['placeholder']['title'])) ? esc_attr($this->field['placeholder']['title']) : __( 'Map Title', 'portfolio' );
                ...
                if ( $this->field[ 'show' ][ 'description' ] ) {
                    $placeholder = (isset($this->field['placeholder']['description'])) ? esc_attr($this->field['placeholder']['description']) : __( 'Address', 'portfolio' );
                ...
                if ( $this->field[ 'show' ][ 'review_link_img' ] ) {             
                    $placeholder = (isset($this->field['placeholder']['review_link_img'])) ? esc_attr($this->field['placeholder']['review_link_img']) : __( 'Review Link Image', 'portfolio' );
